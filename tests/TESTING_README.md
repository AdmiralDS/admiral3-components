...
`generate:component` создаёт матричный каркас с секциями вариантов и состояний. Стартовая секция вариантов перебирает
dimension-константу компонента; после определения API матрицу нужно дополнить всеми appearance и значимыми состояниями.
`validate:components` дополнительно требует разбивку снимков через `VisualSamples` и не принимает одиночный
default-пример без snapshot-рядов.
...
`darkNeutral`); небольшая custom-секция без ряда снимается целиком. Снимки сравниваются пиксель в пиксель с явными
`threshold: 0` и `maxDiffPixels: 0`: тест падает при любом изменившемся пикселе, включая минимальное отличие цвета. На
время снимка Playwright также отключает CSS-анимации и скрывает caret.
